...
-- Step 4: Migrate existing roles from profiles to user_roles (only if not already migrated)
...
-- Step 5: Create RLS policy for user_roles (users can view their own roles)
...
-- Step 6: Create RLS policy for admin access to all roles
...
-- Step 7: Create RLS policy for admin to manage roles
...
-- Step 8: Update educations table RLS policies to use new has_role function
...
-- Step 9: Update premium content access policy
...
-- Step 11: Update premium_packages RLS policies
...
-- Step 12: Update profiles RLS policies for admin access
